...
def send_signup_email_admin(name, receiver):
    ...
    sender = 'austinbrian005@gmail.com'
...
def send_signup_email_resident(name, username, password, admin, hood, receiver):
    ...
    # passing in the context vairables
    text_content = render_to_string('email/email-occupant.txt', {
                                    "name": name, "username": username, "password": password, "admin": admin, "hood": hood})
    html_content = render_to_string('email/email-occupant.html', {
                                    "name": name, "username": username, "password": password, "admin": admin, "hood": hood})
